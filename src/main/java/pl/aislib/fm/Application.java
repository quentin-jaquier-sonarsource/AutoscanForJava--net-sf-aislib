...
public class Application implements Pasture {
  ...
  /**
   * {@link ObjectContainer} object for the application
   */
  ...
  public Application(String _applicationName, HttpServlet _servlet) {
    ...
    pageSelector         = new DefaultPageSelector();
  ...
  private void initializeWorkflow(InputSource workflowSource) throws ApplicationConfigurationException {
    ...
    EntityResolver jarEntityResolver = FmEntityResolver.getResolverInstance(workflowLog);
  ...
  private void initializeMessages(InputSource messagesSource) throws ApplicationConfigurationException {
    ...
    EntityResolver jarEntityResolver = FmEntityResolver.getResolverInstance(messagesLog);
  ...
  private void initializeForms(InputSource formsSource) throws ApplicationConfigurationException {
    ...
    try {
      ...
      throw new ApplicationConfigurationException(saxe.getMessage(), saxe);
    }
    ...
    EntityResolver jarEntityResolver = FmEntityResolver.getResolverInstance(formsLog);
    formsReader.setEntityResolver(jarEntityResolver);
    try {
      formsReader.parse(formsSource);
    } catch (IOException ioe) {
      throw new ApplicationConfigurationException(ioe.getMessage(), ioe);
    } catch (SAXException saxe) {
      if (saxe.getException() != null) {
        throw new ApplicationConfigurationException(saxe.getMessage(), saxe.getException());
      }
      throw new ApplicationConfigurationException(saxe.getMessage(), saxe);
    }
  }
  ...
  public void setConfigAdapter(ConfigAdapter _configAdapter) {
    if (_configAdapter != null) {
      if (mainLog.isInfoEnabled()) {
        mainLog.info("setting config adapter: " + _configAdapter);
      }
  ...
  /**
   * Set {@link PageSelector} for this Application.
   *
   * @param _pageSelector which slould be used for determinig page of user's request.
   */
  public void setPageSelector(PageSelector _pageSelector) {
    if (_pageSelector != null) {
      if (mainLog.isInfoEnabled()) {
        mainLog.info("setting page selector: " + _pageSelector);
      }
      pageSelector = _pageSelector;
    } else {
      if (mainLog.isErrorEnabled()) {
        mainLog.error("try to set null as a page selector");
      }
      throw new NullPointerException("page selector cannot be null");
    }
  }
  ...
  public void dispatch(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    String      pageName  = pageSelector.getRequestedPageKey(request);
    HttpSession session   = request.getSession();

    PageInfo pageInfo = controller.getPageInfo(request, response, pageSelector);
  ...
  /**
   * Set {@link ObjectContainer} for this Application.
   *
   * @param container
   */
  public void setObjectContainer(ObjectContainer container) {
    if (container != null) {
      if (mainLog.isInfoEnabled()) {
        mainLog.info("setting object container: " + container);
      }
      objectContainer = container;
    } else {
      if (mainLog.isErrorEnabled()) {
        mainLog.error("try to set null as a object container");
      }
      throw new NullPointerException("object container cannot be null");
    }
  }
   ...
   *
   * Call {@link HttpServlet#getInitParameter(String)} and {@link javax.servlet.ServletContext#getInitParameter}
   * methods.
   */
  ...
  class JNDIObjectContainer implements ObjectContainer {
    public Object getObject(String name, Class clazz) throws Exception {
      String className = (String) new InitialContext().lookup("java:comp/env/" + name);
      if (className != null) {
