...
import pl.aislib.util.web.servlet.BaseServlet;
...
public class SpyServlet extends BaseServlet {
  ...
  private static final String JNDI_WALKER = "/jndiwalker";
  ...
  public void doPostOrGet(HttpServletRequest request, HttpServletResponse response)
    ...
    } else if (pathInfo.startsWith(JNDI_WALKER)) {
      writer.write(jndiWalker(request, response));
    } else if (action == null) {
  ...
  public String getUsage(HttpServletRequest request, HttpServletResponse response) {
    ...
    // JNDI Walker
    result.append("  <li class='option'><a class='link' href='");
    result.append(response.encodeURL(request.getRequestURI() + JNDI_WALKER));
    result.append("'>JNDI Walker</a></li>\n");
  ...
  private String jndiWalker(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      result.append("<h3>");
      result.append( (ctx == null) ? "null" : ctx.getEnvironment().toString() );
      result.append("</h3>\n");
      tree = request.getParameter("tree");
      result.append("<table>\n");
      result.append("  <tr><td>Name</td><td>Class</td><td>Value</td></tr>\n");
      if ( tree != null) {
        NamingEnumeration attrs = ctx.listBindings(tree);
        while (attrs.hasMore()) {
          result.append("  <tr>\n");
          Binding binding = (Binding) attrs.next();
          result.append("    <td>");
          if (binding.getObject() instanceof Context) {
            result.append("<a href='");
            result.append(encodedURL + "?tree=" + tree + "/" + binding.getName());
            result.append("'>" + binding.getName() + "</a>");
          } else {
            result.append("<b>" + binding.getName() + "</b>");
          }
          result.append("</td>\n    <td>");
          result.append(binding.getClassName());
          result.append("</td>\n    <td>");
          result.append(binding.getObject());
          result.append("</td>\n  </tr>\n");
        }
      }
    } catch (Exception e) {
      result = new StringBuffer();
      result.append("<h3>Exception occred: </h3>");
    ...
    result.append("      <form method='post' action='" + request.getRequestURI() + "'>\n");
    result.append("        <input type='text' name='tree' size='50' value='");
    result.append((tree != null) ? tree : "");
    result.append("'/>\n        <input type='submit'/>\n");
    result.append("      </form>\n    </td>\n  </tr>\n");
    result.append("</table>\n");
