package pl.aislib.util.crypt;

/**
 * Widely used <code>unix crypt</code> algorithm.
 *
 * @author unknown
...
public class UnixCrypt {
  ...
  private static final int dEncrypt(int l, int r, int s, int e0, int e1, int ss[]) {
    int t, u, v;
  ...
  public static final String crypt(String original) {
    ...
    salt =
      (new StringBuffer())
        .append(SALT_CHARS[Math.abs(randomGenerator.nextInt()) % numSaltChars])
        .append(SALT_CHARS[Math.abs(randomGenerator.nextInt()) % numSaltChars])
        .toString();
