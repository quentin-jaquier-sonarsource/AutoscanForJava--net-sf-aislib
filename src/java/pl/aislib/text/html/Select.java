...
public class Select extends AbstractHTMLObject {
  ...
  public Select() {
    ...
    try {
      EnumeratedAttribute enum = new EnumeratedAttribute("disabled");
      enum.addAllowedValue("disabled");
      selectAttrs.add(enum);

      enum = new EnumeratedAttribute("multiple");
      enum.addAllowedValue("multiple");
      selectAttrs.add(enum);
